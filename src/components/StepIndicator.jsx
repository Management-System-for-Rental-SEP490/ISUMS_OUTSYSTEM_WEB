...
// Icons live at module scope (they never change); labels are resolved via
// i18n at render time so the step strip follows the active locale.
const STEP_ICONS = [
  (
...
export default function StepIndicator({ currentStep, onGoToStep }) {
  ...
  const STEPS = STEP_ICONS.map((icon, i) => ({
    label: t(`stepIndicator.step${i + 1}`),
    icon,
  }));
  return (
          ...
          <React.Fragment key={n}>
            ...
            <div className="flex flex-col items-center gap-2 min-w-0">
              <button
                ...
                disabled={!canBack}
                title={canBack ? t("stepIndicator.backTo", { label }) : undefined}
                className={[
